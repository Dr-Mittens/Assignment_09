...
# Change Log: (James Miller, 3/29/22, Added track compatibility)
...
class FileIO:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        try:
            ...
            cdfile.close
            print('There was a error with saving the CDs!', e, e.__doc__, type(e), sep='\n')
        try:
            ...
            with open(file_name_trk, 'w') as file:
                for disc in lst_Inventory:
                    ...
                    for item in tracks:
                        cd_source = disc.cd_id
                        if not item == None:
                            adding = '{},{}'.format(cd_source,item.get_record())
                            file.write(adding)
        except Exception as e:
            print('There was a error with saving the Tracks!', e, e.__doc__, type(e), sep='\n')
        ...
        try:
            ...
            with open(file_name_trk, 'r') as file:
                for row in file:
                    ...
                    targetcd = PC.DataProcessor.select_cd(lst_Inventory,int(item[0]))
                    newtrack = DC.Track(int(item[1]),item[2],item[3])
                    targetcd.add_track(newtrack)
